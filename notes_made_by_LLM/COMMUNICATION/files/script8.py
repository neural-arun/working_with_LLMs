...
def draw_knowledge_module_page(c, week):
    ...
    for i, resource in enumerate(module["learning_resources"], 1):
        c.drawString(MARGIN + 0.2*inch, y, f"{i}. {resource}")
        y -= 0.25*inch
    ...
    # Goal template
    goal_template = GOAL_TEMPLATES[week]
    c.setFont("Helvetica", 10)
    c.setFillColor(COLOR_TEXT_BODY)
    
    # Word wrap the goal template
    words = goal_template.split()
    line = ""
    for word in words:
        if len(line + word) < 90:
            line += word + " "
            ...
            line = word + " "
    if line:
        c.drawString(MARGIN, line.strip())
        y -= 0.2*inch
...
def draw_progressive_challenge_box(c, y_pos, week, day):
    ...
    challenge = PROGRESSIVE_CHALLENGES[week][day-1]
    ...
    c.drawString(MARGIN + 0.2*inch, y_pos - 0.6*inch, challenge)
...
def create_progressive_social_mastery_journal():
    ...
    print(f"📊 Generated comprehensive progressive social mastery system:")
    print(f"   • 7 Knowledge Module pages (30-60 min learning each)")
    print(f"   • 49 Daily Practice pages (progressive difficulty)")
    print(f"   • 7 Weekly Review pages (quantitative + qualitative)")
    print(f"   • 2 Assessment pages (intro + final transformation)")
    print(f"   • Total: {7 + 49 + 7 + 2} pages")
    print("🚀 Your systematic path from introvert to social mastery is ready!")
    print("💡 Key improvements implemented:")
    print("   ✅ Progressive difficulty scaffolding")
    print("   ✅ Knowledge modules with learning resources")
    print("   ✅ Concrete, measurable success metrics")
    print("   ✅ Personalized goal templates")
    print("   ✅ Career-focused social skill development")

if __name__ == "__main__":
    create_progressive_social_mastery_journal()