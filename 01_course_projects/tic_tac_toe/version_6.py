...
class TicTacToeGame:
    def __init__(self, master):
        ...
        self.master.configure(bg="#2d2f34")
        ...
        self.top_frame = tk.Frame(self.master, bg="#2d2f34")
        self.top_frame.pack(pady=20)
        self.grid_frame = tk.Frame(self.master, bg="#2d2f34")
        self.grid_frame.pack()
        self.bottom_frame = tk.Frame(self.master, bg="#2d2f34")
        self.bottom_frame.pack(pady=20)

        # Status and score labels
        self.status_label = tk.Label(self.top_frame, text="Player X's turn", font=('Arial', 24, 'bold'), fg="white", bg="#2d2f34")
        self.status_label.pack(side=tk.TOP)
        
        self.score_label = tk.Label(self.top_frame, text="X: 0 | O: 0 | Draws: 0", font=('Arial', 14), fg="#c0c0c0", bg="#2d2f34")
        self.score_label.pack(side=tk.BOTTOM, pady=(5, 0))
        
        # Game board buttons
        self.buttons = []
        for i in range(9):
            row, col = divmod(i, 3)
            # Style the buttons to look like the image
            button = tk.Button(self.grid_frame, text='', font=('Arial', 48, 'bold'), width=3, height=1,
                               fg="#e0e0e0", bg="#3c3a42", activebackground="#4a4d52",
                               bd=5, relief=tk.SUNKEN, command=lambda i=i: self.handle_click(i))
            button.grid(row=row, column=col, padx=5, pady=5)
            self.buttons.append(button)
            
        # Controls and options
        self.control_frame = tk.Frame(self.bottom_frame, bg="#2d2f34")
        self.control_frame.pack()
        
        # Updated style for the New Game button (orange)
        self.new_game_button = tk.Button(self.control_frame, text="New Game", command=self.reset_game,
                                         font=('Arial', 14, 'bold'), relief=tk.RAISED, bd=4, padx=10, pady=5,
                                         bg="#f2994a", fg="black", activebackground="#f5b978")
        self.new_game_button.pack(side=tk.LEFT, padx=10)
        
        # Updated style for difficulty selection (gray)
        ...
        self.difficulty_menu.config(font=('Arial', 14, 'bold'), relief=tk.RAISED, bd=4,
                                   bg="#75777c", fg="black", activebackground="#8d8f94")
        self.difficulty_menu.pack(side=tk.LEFT, padx=10)
        
        # Updated style for game mode selection (green)
        ...
        self.mode_menu.config(font=('Arial', 14, 'bold'), relief=tk.RAISED, bd=4,
                             bg="#27ae60", fg="black", activebackground="#43c473")
    ...
    def handle_click(self, index):
        ...
        if self.game_active and is_valid_move(self.board, index):
            ...
            if self.current_player == 'X':
                self.buttons[index].config(text='✕', fg='#e74c3c', state=tk.DISABLED) # Red cross
            else:
                self.buttons[index].config(text='✓', fg='#2ecc71', state=tk.DISABLED) # Green check
    ...
    def ai_move(self):
        ...
        if self.difficulty == 'easy':
            ...
            human_player = '✔' if self.ai_player == 'X' else 'X'
        ...
        if is_valid_move(self.board, move):
            ...
            if self.current_player == 'X':
                self.buttons[move].config(text='✕', fg='#e74c3c', state=tk.DISABLED) # Red cross
            else:
                self.buttons[move].config(text='✓', fg='#2ecc71', state=tk.DISABLED) # Green check
    ...
    def update_score_label(self):
        self.score_label.config(text=f"X: {self.scores['X_wins']} | ✔: {self.scores['O_wins']} | Draws: {self.scores['draws']}")
    ...
    def start_game(self):
        ...
        if self.mode == '1-player':
            self.ai_player = '✔' if self.current_player == 'X' else 'X'
    ...
    def update_ui(self):
        ...
        for i, button in enumerate(self.buttons):
            if self.board[i] == 'X':
                button.config(text='✕', fg='#e74c3c', state=tk.NORMAL) # Red cross
            elif self.board[i] == '✔':
                button.config(text='✓', fg='#2ecc71', state=tk.NORMAL) # Green check
            else:
                button.config(text='', state=tk.NORMAL, fg="#e0e0e0") # Empty
    ...
    def reset_game(self):
        ...
        """
        self.board = create_board()
        self.current_player = random.choice(['X', '✔'])
        self.game_active = True
        self.update_ui()
        self.update_score_label()
