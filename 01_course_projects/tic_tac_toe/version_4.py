...
class TicTacToeGame:
    ...
    def handle_click(self, index):
        ...
        if self.game_active and is_valid_move(self.board, index):
            ...
            if self.current_player == 'X':
                # This is where the X color is set
                self.buttons[index].config(text='✕', fg='red', state=tk.DISABLED)
            else:
                # This is where the O color is set
    ...
    def ai_move(self):
        ...
        if is_valid_move(self.board, move):
            ...
            if self.current_player == 'X':
                # This is where the AI's X color is set
                self.buttons[move].config(text='✕', fg='red', state=tk.DISABLED)
            else:
                # This is where the AI's O color is set
    ...
    def update_ui(self):
        ...
        for i, button in enumerate(self.buttons):
            # This is the key change to make sure the text and color are correct
            # when the board is reset.
            if self.board[i] == 'X':
                button.config(text='✕', fg='red', state=tk.NORMAL, bg=self.master.cget('bg'))
            elif self.board[i] == 'O':
                button.config(text='✓', fg='green', state=tk.NORMAL, bg=self.master.cget('bg'))
            else:
                button.config(text='', state=tk.NORMAL, bg=self.master.cget('bg'), fg='black')
